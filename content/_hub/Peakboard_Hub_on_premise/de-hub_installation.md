...
In diesem Schritt legst du fest, gegen welches Backend der Peakboard Hub on premise die Logins prüft. Die Wahl entscheidet, woher die Identitäten deiner Anwender kommen — vom lokalen Windows des Servers oder aus dem Active Directory deiner Domäne.

* **Local Users** (1) — Die Anmeldung erfolgt gegen die *lokalen Windows-Benutzerkonten* des Servers, auf dem der Peakboard Hub on premise installiert ist. Diese Konten und ihre Passwörter werden in Windows verwaltet (zum Beispiel über die Windows-Computerverwaltung), nicht im Peakboard Hub. Es besteht keine Abhängigkeit zu einem Active Directory; der Server muss kein Domänenmitglied sein. Beim Login gibst du den *Windows-Benutzernamen* sowie das zugehörige *Windows-Passwort* ein. Empfehlenswert für eigenständige Server, Evaluierungs- und Testumgebungen sowie kleine Teams ohne eigene Domäne.
* **AD Users** (2) — Anmeldung über den Windows Domain Controller deiner Umgebung (Active Directory). Vorausgesetzt, der Server, auf dem der Peakboard Hub on premise installiert wird, ist Mitglied derselben Domäne. Beim Login gibst du den *Windows Nutzernamen ohne Domänenpräfix* sowie das *Windows Passwort* ein, sodass deine Anwender mit ihren gewohnten Unternehmenszugangsdaten arbeiten.

In beiden Varianten gilt: Der Peakboard Hub on premise speichert selbst keine Passwörter und legt in der [Nutzerverwaltung](/hub/de-hub_usermanagement.html) keine Konten mit Initialpasswort an. Das Benutzerkonto im Peakboard Hub entsteht automatisch beim ersten erfolgreichen Login. Der erste Nutzer, der sich anmeldet, wird dabei automatisch zum Administrator. In der Hub-Nutzerverwaltung pflegst du anschließend ausschließlich Gruppen und Berechtigungen — die eigentlichen Zugangsdaten verbleiben bei Windows beziehungsweise im Active Directory.
