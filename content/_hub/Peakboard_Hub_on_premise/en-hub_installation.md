...
If you deliberately kept the database and the .NET Hosting Bundle, both are still available. You can re-install the Peakboard Hub on premise from the setup file at any time and pick **Use an existing database and user** on the **Database** step to reuse your previous data set.
